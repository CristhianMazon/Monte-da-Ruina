...
import { Flame, X } from "lucide-react"; // Ícones para o modal

const GameScreen = () => {
    // Adicionei 'winStreak' aqui
    const { balance, placeBet, winStreak } = useGame();
    ...
    // Estado para o Modal da Ganância
    const [showGreedModal, setShowGreedModal] = useState(false);

    const showVictory = isRevealed && message.includes("VITORIA");

    // --- LÓGICA DO GATILHO DA GANÂNCIA ---
    useEffect(() => {
        // Se ganhou a 3ª seguida e a revelação acabou
        if (isRevealed && winStreak === 3) {
            // Pequeno delay para o jogador ver a vitória antes do desafio
    ...
    const resetRound = () => {
        ...
        setMessage("");
        setShowGreedModal(false); // Garante que fecha se resetar manual
    };
                ...
                {RISK_CARDS.map((card) => {
                    ...
                    return (
                        <button
                            ...
                            `}
                        >
                            <div className="absolute top-2 left-2 w-2 h-2 rounded-full bg-black/30 shadow-inner"></div>
                            <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-black/30 shadow-inner"></div>
                            <div className="absolute bottom-2 left-2 w-2 h-2 rounded-full bg-black/30 shadow-inner"></div>
                            <div className="absolute bottom-2 right-2 w-2 h-2 rounded-full bg-black/30 shadow-inner"></div>
